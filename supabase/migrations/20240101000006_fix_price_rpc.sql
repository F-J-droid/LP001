...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      v.sku, v.is_active, 
      CASE 
        WHEN v_payment_method = 'pix' AND p.pix_price_cents IS NOT NULL THEN p.pix_price_cents
        WHEN p.sale_price_cents IS NOT NULL THEN p.sale_price_cents
        ELSE p.regular_price_cents
      END,
  ...
  LOOP
    ...
    BEGIN
      INSERT INTO public.orders (
        ...
        status, payment_status, payment_method, subtotal_cents, discount_cents, shipping_cents, total_cents,
        ...
        'pending_payment', 'pending', v_payment_method, v_subtotal, 0, v_shipping_cents, v_total,
